...
router.get('/cargos/:empId', verificarToken, (req, res) => {
    const emp = db.prepare('SELECT * FROM emprendimientos WHERE id = ? AND usuario_id = ?').get(req.params.empId, req.usuario.id);
    if (!emp) return res.status(403).json({ error: 'Acceso denegado' });
    const cargos = db.prepare('SELECT * FROM cargos WHERE emprendimiento_id = ? ORDER BY nombre').all(req.params.empId);
...
router.post('/cargos', verificarToken, (req, res) => {
    ...
    const emp = db.prepare('SELECT * FROM emprendimientos WHERE id = ? AND usuario_id = ?').get(emprendimiento_id, req.usuario.id);
    if (!emp) return res.status(403).json({ error: 'Acceso denegado' });
    const result = db.prepare('INSERT INTO cargos (emprendimiento_id, nombre, salario_base, descripcion) VALUES (?, ?, ?, ?)').run(emprendimiento_id, nombre, parseFloat(salario_base), descripcion || '');
    res.json({ id: result.lastInsertRowid, mensaje: 'Cargo creado correctamente' });
});

router.delete('/cargos/:id', verificarToken, (req, res) => {
    const trabajadores = db.prepare('SELECT COUNT(*) as total FROM trabajadores WHERE cargo_id = ?').get(req.params.id);
...
router.post('/trabajadores', verificarToken, (req, res) => {
    const { emprendimiento_id, cargo_id, nombre, apellido, edad, cedula, fecha_ingreso } = req.body;
    ...
    if (edad !== undefined && edad !== null && edad !== '') {
        ...
        if (isNaN(edadNum) || edadNum < 14 || edadNum > 99)
            return res.status(400).json({ error: 'La edad debe ser un número entre 14 y 99' });
    }
    const result = db.prepare(
        'INSERT INTO trabajadores (emprendimiento_id, cargo_id, nombre, apellido, edad, cedula, fecha_ingreso) VALUES (?, ?, ?, ?, ?, ?, ?)'
    ).run(emprendimiento_id, cargo_id, nombre, apellido, edad ? Number(edad) : null, cedula || '', fecha_ingreso || '');
...
router.put('/trabajadores/:id', verificarToken, (req, res) => {
    const { cargo_id, nombre, apellido, edad, cedula, fecha_ingreso, estado } = req.body;
    ...
    db.prepare('UPDATE trabajadores SET cargo_id=?, nombre=?, apellido=?, edad=?, cedula=?, fecha_ingreso=?, estado=? WHERE id=?')
        .run(cargo_id || t.cargo_id, nombre || t.nombre, apellido || t.apellido, edad !== undefined ? Number(edad) : t.edad, cedula ?? t.cedula, fecha_ingreso ?? t.fecha_ingreso, estado ?? t.estado, req.params.id);
...
router.get('/resumen/:empId', verificarToken, (req, res) => {
    const emp = db.prepare('SELECT * FROM emprendimientos WHERE id = ? AND usuario_id = ?').get(req.params.empId, req.usuario.id);
    if (!emp) return res.status(403).json({ error: 'Acceso denegado' });
    const totalTrabajadores = db.prepare("SELECT COUNT(*) as total FROM trabajadores WHERE emprendimiento_id = ? AND estado = 'activo'").get(req.params.empId).total;
